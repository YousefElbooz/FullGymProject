...
void FileHandler::saveClasses(const QString& filePath, const QMap<int, GymClass*>& gymClasses) {
    ...
    for (auto it = gymClasses.begin(); it != gymClasses.end(); ++it) {
        ...
        // Write waitlist data
        saveWaitlistData(out, gymClass->getWaitlistAsVector());
...
QMap<int, GymClass*> FileHandler::loadClasses(const QString& filePath, QMap<int, GymClass*>& gymClasses, const QMap<int, Member*>& members, const QMap<int, Staff*>& staffMap) {
    ...
    while (!in.atEnd()) {
        ...
        // Handle waitlist entries - now with combined format
        if (line.startsWith("Waitlist:")) {
            if (gc) {
                QStringList entries = line.mid(QString("Waitlist:").length()).split(",", Qt::SkipEmptyParts);
                for (const QString& entry : entries) {
                    QStringList parts = entry.split(":", Qt::SkipEmptyParts);
                    if (parts.size() >= 2) {
                        int memberId = parts[0].toInt();
                        bool isVip = parts[1].toLower() == "true";
                        if (members.contains(memberId)) {
                            Member* member = members[memberId];
                            // Make sure VIP status is set correctly
                            member->setIsVip(isVip); 
                            gc->addToWaitlist(member);
                        }
                    }
                }
            }
        ...
        if (line.startsWith("VIPWaitlist:")) {
            if (gc) {
                QStringList memberIds = line.mid(QString("VIPWaitlist:").length()).split(",", Qt::SkipEmptyParts);
                for (const QString& idStr : memberIds) {
                    int memberId = idStr.toInt();
                    if (members.contains(memberId)) {
                        Member* member = members[memberId];
                        member->setIsVip(true); // Ensure VIP status
                        gc->addToWaitlist(member);
                    }
                }
            }
        ...
        if (line.startsWith("NormalWaitlist:")) {
            if (gc) {
                ...
                for (const QString& idStr : memberIds) {
                    ...
                    if (members.contains(memberId)) {
                        Member* member = members[memberId];
                        member->setIsVip(false); // Ensure normal status
                        gc->addToWaitlist(member);
                    }
                }
            }
...
void FileHandler::saveWaitlistData(QTextStream &out, const QVector<Member*>& waitlistMembers) {
    if (!waitlistMembers.isEmpty()) {
        out << "Waitlist:";
        for (const Member* member : waitlistMembers) {
            out << member->getId() << ":" << (member->getIsVip() ? "true" : "false") << ",";
        }
        out << "\n";
    }
}
